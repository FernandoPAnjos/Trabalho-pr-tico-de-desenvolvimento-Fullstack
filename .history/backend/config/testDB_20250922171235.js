...
async function testConnection() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('produtos') // <-- MUDE AQUI SE NECESSÁRIO
      .select('*')
      .limit(1);
    ...
    console.log('✅ Conexão e consulta ao Supabase bem-sucedidas!');
    console.log('Dados recebidos (pode ser um array vazio se a tabela não tiver registros):', data);

  } catch (err) {
    // Captura qualquer outro erro inesperado (ex: de rede, etc.)
    console.error('❌ Falha geral ao tentar executar o teste:', err);
  } finally {
