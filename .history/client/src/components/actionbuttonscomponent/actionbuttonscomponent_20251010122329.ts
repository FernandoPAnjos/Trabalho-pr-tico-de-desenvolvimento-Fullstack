  ...
  abrirDialogAdicionar(): void {
   ...
   deletarSelecionados(): void {
    ...
    if (confirmacao) {
      ...
      // Promise.all espera todas as exclusões terminarem
      Promise.all(deletePromises)
        .then(() => {
          alert('Usuários excluídos com sucesso!');
          this.selection.clear(); // Limpa a seleção visual
          this.usersDeleted.emit(); // 4. Avisa o AppComponent para recarregar a lista
        })
        .catch(err => {
          console.error("Ocorreu um erro ao deletar usuários", err);
          alert("Ocorreu um erro ao deletar um ou mais usuários.");
        });
    }
  }
